...
export default class DxfComparePanel {
  ...
  buildPanel() {
    ...
    header.innerHTML = `_(${changesValues.length})`;

    const addedChangesValues = changesValues.filter(
      (val) => val.type === "Added"
    );
    const deletedChangeValues = changesValues.filter(
      (val) => val.type === "Removed"
    );
    const modifiedChanageValues = changesValues.filter(
      (val) => val.type === "Modified"
    );

    if (addedChangesValues.length > 0) {
      this.buildList(addedChangesValues, "_");
    }

    if (deletedChangeValues.length > 0) {
      this.buildList(deletedChangeValues, "_");
    }

    if (modifiedChanageValues.length > 0) {
      this.buildList(modifiedChanageValues, "_");
    }
  }
